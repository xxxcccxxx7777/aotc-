...
namespace AotC.Content
{
  internal class ArkoftheCosmosSwungBlade : ModProjectile
  {
    ...
    public virtual bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
    {
      ...
      Vector2 vector2_1 = Vector2.op_Multiply(((Vector2) ref distanceFromPlayer).Length(), Utils.ToRotationVector2(this.Projectile.rotation));
    ...
    public void DrawSingleSwungScissorBlade(Color lightColor)
    {
      ...
      ((Vector2) ref vector2_1).\u002Ector(flag ? (float) texture2D1.Width : 0.0f, (float) texture2D1.Height);
    ...
    public void DoParticleEffects(bool swirlSwing)
    {
      ...
      if (Utils.NextBool(Main.rand))
      {
        ...
        Vector2.op_Multiply(Vector2.op_Multiply(Utils.SafeNormalize(Utils.RotatedBy(vector2, 1.57079637050629 * (double) ((Entity) this.Owner).direction, new Vector2()), Vector2.Zero), 2f), (float) (1.0 + (double) ((Vector2) ref vector2).Length() / 15.0));
      }
