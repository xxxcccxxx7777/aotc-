    ...
    public virtual void AI()
    {
      ...
      else
      {
        ...
        if ((double) ((Entity) this.Projectile).velocity.AngleBetween(Vector2.op_Subtraction(((Entity) this.target).Center, ((Entity) this.Projectile).Center)) < 3.14159274101257)
        {
          ...
          ((Entity) this.Projectile).velocity = Vector2.op_Multiply(Vector2.op_Multiply(Utils.ToRotationVector2(Utils.AngleTowards(Utils.ToRotation(((Entity) this.Projectile).velocity), num, this.HomingStrenght)), ((Vector2)  ((Entity) this.Projectile).velocity).Length()), 0.995f);
        }
      }
